...
const WC_BASE_URL = process.env.WC_BASE_URL || 'https://velosclothing.com';
...
export function mapProduct(product) {
  ...
  // Extraer tallas y colores desde atributos de WooCommerce
  const attributes = product?.attributes || [];
  const sizeAttr = attributes.find((attr) => 
    ['talla', 'size', 'tallas'].includes(String(attr.name || '').toLowerCase())
  );
  const colorAttr = attributes.find((attr) => 
    ['color', 'colores', 'colours'].includes(String(attr.name || '').toLowerCase())
  );

  const sizes = sizeAttr?.options || acf.size || acf.talla || [];
  const colors = colorAttr?.options || acf.color || acf.colores || [];
